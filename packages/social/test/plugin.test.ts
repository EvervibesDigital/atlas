...
describe("social plugin — getConnectUrl", () => {
  it("returns a real Meta OAuth URL built from META_APP_ID, SOCIAL_LOGIN_CONFIG_ID, and a generated state", async () => {
    const atlas = new Atlas({
      guardian: permissiveGuardian(),
      config: new ConfigVault({ META_APP_ID: "app-123", SOCIAL_LOGIN_CONFIG_ID: "cfg-456" }),
    });
    ...
    expect(result.url).toContain("config_id=cfg-456");
    expect(result.url).toContain("state=");
  });

  it("throws a clear error when META_APP_ID isn't configured", async () => {
    const atlas = new Atlas({ guardian: permissiveGuardian(), config: new ConfigVault({ SOCIAL_LOGIN_CONFIG_ID: "cfg-456" }) });
  ...
  it("throws a clear error when SOCIAL_LOGIN_CONFIG_ID isn't configured", async () => {
    const atlas = new Atlas({ guardian: permissiveGuardian(), config: new ConfigVault({ META_APP_ID: "app-123" }) });
    await atlas.use(createSocialPlugin({ redirectUri: "https://x/cb" }));

    await expect(atlas.invoke("social", { op: "getConnectUrl" })).rejects.toThrow(/SOCIAL_LOGIN_CONFIG_ID/);
  });
});
